...
public class PriorityQueueSerde<T> implements Serde<PriorityQueue<T>> {
    ...
    public PriorityQueueSerde(final Comparator<T> comparator, final Serde<T> SerDe) {
        inner = Serdes.serdeFrom(new PriorityQueueSerializer<>(comparator, SerDe.serializer()),
                new PriorityQueueDeserializer<>(comparator, SerDe.deserializer()));
    }
